...
if ($dateFilter !== 'all') {
    ...
    switch ($dateFilter) {
        case 'today':
            $dateCondition = " AND DATE(pi.created_at) = CURDATE()";
            break;
        case 'yesterday':
            $dateCondition = " AND DATE(pi.created_at) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)";
            break;
        case 'week':
            $dateCondition = " AND pi.created_at >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
            break;
        case 'month':
            $dateCondition = " AND pi.created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)";
            break;
        case 'custom':
            if (!empty($startDate) && !empty($endDate)) {
                $dateCondition = " AND DATE(pi.created_at) BETWEEN '$startDate' AND '$endDate'";
            }
            break;
    }
}
    ...
    <header>
        <div class="brand">
            ...
            <div>
                ...
                <div class="crumbs">
                    <a href="analytics.php">Analytics</a> &nbsp;›&nbsp; <strong>Product Sales Analysis</strong>
                </div>
            </div>
        </div>
    </header>
